...
var selectedProductLinkID = {/literal}{$linkedproductid}
{if $canlink or $companycode != ''}
...
function initialize(pParams)
{
	...
	{
		Ext.MessageBox.show({ title: "{/literal}{#str_TitleError#}{literal}", msg: "Could Not Connect", buttons: Ext.MessageBox.OK, icon: Ext.MessageBox.WARNING });
	}
	...
	{
		var submitURL = 'index.php?fsaction=AdminProducts.saveProductConfig&ref={/literal}{$ref}{literal}';
		...
		if (submit)
		{
			Ext.taopix.formPanelPost(fp, form, postParamArray, submitURL, "{/literal}{#str_MessageSaving#}{literal}", saveCallback);
		}
	}
	...
	function onDeleteTreeItems()
	{
		...
		{
			var message = "{/literal}{#str_ConfirmDeleteSection#}{literal}";
		}
		else
		{
			var message = "{/literal}{#str_ConfirmDeleteComponent#}{literal}";
		}

		Ext.MessageBox.confirm("{/literal}{#str_LabelConfirmation#}{literal}", message, onDeleteTreeItemsResult);
	}
	...
	{
		var message = "{/literal}{#str_ConfirmDeleteComponentPrice#}{literal}";

		Ext.MessageBox.confirm("{/literal}{#str_LabelConfirmation#}{literal}", message, onDeletePriceResult);
	}
	...
	function editConfigHandler(btn)
	{
		if (btn.id == 'editconfig')
		{
			...
			{
				Ext.MessageBox.show({ title: "{/literal}{#str_TitleError#}{literal}", msg: "{/literal}{#str_ErrorPriceLinkCorruptionDetected#}{literal}", buttons: Ext.MessageBox.OK, icon: Ext.MessageBox.WARNING, maxWidth: 500});
			}
			...
			{/literal} 
			{*only enable the linking combo if it is valid for linking and we are not on a site*}
			{if $canlink != 0}
		...
		else
		{
			...
			Ext.Ajax.request({
				url: 'index.php?fsaction=AdminProducts.getLinkedProductCode&ref={/literal}{$ref}{literal}',
				success: getLinkedProductCodeSuccess,
				failure: getLinkedProductCodeFailure,
				params: { productcode: '{/literal}{$productcode}{literal}'},
				method: 'GET'
			});

			Ext.Ajax.request({
				   url: 'index.php?fsaction=AdminProducts.refreshProductTree&ref={/literal}{$ref}{literal}',
				   success: productTreeRefreshSuccess,
				   failure: productTreeRefreshFailure,
				   params: { productid: {/literal}{$productid}{literal}, companycode: selectedCompanyCode, getLinkedTree: 1},
				   method: 'GET'
				});
		}
	}
	...
	function onPricingEdit(btn, ev)
	{
		...
		gPricingEditWindow = new Ext.Window({
				...
				[
					...
					{ text: "{/literal}{#str_ButtonCancel#}{literal}", handler: function(){ gPricingEditWindow.close();} },
					{ text: "{/literal}{#str_ButtonAdd#}{literal}", id: 'nodePricingAddEditButton', handler: priceScreenSaveHandler }
				]
			});
		...
		var newPricingTaopixPanel =
	    {
	        ...
	        licenseKeyStoreURL: 'index.php?fsaction=AdminProductPricing.getLicenseKeyFromCompany&ref={/literal}{$ref}{literal}&isedit='+isEdit+'&filterkeylist='+filterKeyList+'&componentcode='+componentCode+'&productcode={/literal}{$productcode}{literal}&id='+ parentid +'&companycode={/literal}{$companycode}{literal}',
	        ...
	        images: {deleteImg: '{/literal}{$webroot}{literal}/utils/ext/images/silk/delete.png', addimg: '{/literal}{$webroot}{literal}/utils/ext/images/silk/add.png'},
		...
		if (parentid == 0)
		{
			Ext.getCmp('nodePricingAddEditButton').text = "{/literal}{#str_ButtonAdd#}{literal}";
			Ext.getCmp('nodePricingAddEditButton').setHandler(priceScreenSaveHandler);
		}
		else
		{
			Ext.getCmp('nodePricingAddEditButton').text = "{/literal}{#str_ButtonUpdate#}{literal}";
			...
			for (var i = 0; i < len; i++)
			{
				...
				if ((paramArray[pPriceLinkParentIDArray[i]] != undefined) && (paramArray[pPriceLinkParentIDArray[i]].price != ''))
				{
					...
					for (var j = 0; j < groupCodesArray.length; j++)
					{
						...
						{
							groupCodesArray[j] = '<i>{/literal}{#str_LabelDefault#}{literal}</i>';
						}
	...
	function createDummyPriceLinkeRecordForNode(pNode)
	{
		...
		paramArray[dummyPriceLinkID].productcode = '{/literal}{$productcode}{literal}';
	...
	function defaultPriceRenderer(value, p, record)
	{
		...
		if (value == '')
		{
			...
			return '{/literal}<span '+className+">Default Component Price</span>{literal}";
		}
		else
		{
	...
	function statusRenderer(value, p, record)
	{
		...
		else
		{
			...
			{
				className =  'class = "inactive"';
				return '{/literal}<span '+className+">{#str_LabelActive#}</span>{literal}";
			}
			else
			{
				if ((record.data.companycode == '') && (companyLogin))
				{
					className =  'class = "inactive"';
					return '<span '+className+'>{/literal}{#str_LabelActive#}{literal}</span>';
				}
				else
				{
	...
	function updatePricingForNode(pPriceLinkID, pUpDateToCurrentPriceRecord)
	{
		...
		var priceListID = 0;
		var price = '';
		var productCode = '{/literal}{$productcode}{literal}';
		var categoryCode = '';
		var pricingModel = '';
		var priceAdditionalInfo = '';
		var inheritParentQty = '0';
		...
		else
		{
			...
			if (selectedPricingModel == 3)
			{
				...
				gridColumns = [
				               	...
				               	{header: "{/literal}{#str_LabelProduct#}{literal}", width: 150, hidden: true, renderer: defaultPriceRenderer, dataIndex: 'productcode'},
				               	{header: "{/literal}{#str_LabelCompany#}{literal}", width: 150, hidden: companyColumnHidden, renderer: companyCodeRenderer, dataIndex: 'companycode'},
				    			{header: "{/literal}{#str_LabelLicenseKey#}{literal}", width: 150, dataIndex: 'lkey', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeStart#}{literal}", width: 130, dataIndex: 'qrs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeEnd#}{literal}", width: 130, dataIndex: 'qre', renderer: generalColumnRenderer},
								...
								{id: 'includestax', header: "{/literal}{#str_LabelIncludesTax#}{literal}", width: 160, dataIndex: 'it', renderer: generalColumnRenderer},
								{id: 'active', header: "{/literal}{#str_LabelStatus#}{literal}", renderer: statusRenderer, width: 50, dataIndex: 'active'}
				              ];
			}
			else if (selectedPricingModel == 5)
			{
				...
				gridColumns = [
				               	...
				               	{header: "{/literal}{#str_LabelCompany#}{literal}", width: 150, hidden: companyColumnHidden, renderer: companyCodeRenderer, dataIndex: 'companycode'},
				    			{header: "{/literal}{#str_LabelLicenseKey#}{literal}", width: 150, dataIndex: 'lkey', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeStart#}{literal}", width: 105, dataIndex: 'qrs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeEnd#}{literal}", width: 105, dataIndex: 'qre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PageCountRangeStart#}{literal}", width: 120, dataIndex: 'prs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PageCountRangeEnd#}{literal}", width: 120, dataIndex: 'pre', renderer: generalColumnRenderer},
								...
								{header: "{/literal}{#str_LabelLineSubtract#}{literal}", width: 90, dataIndex: 'ls', renderer: generalColumnRenderer},
								{id: 'includestax', header: "{/literal}{#str_LabelIncludesTax#}{literal}", width: 160, dataIndex: 'it', renderer: generalColumnRenderer},
								{id: 'active', header: "{/literal}{#str_LabelStatus#}{literal}", renderer: statusRenderer, width: 50, dataIndex: 'active'}
				              ];
			}
			else if (selectedPricingModel == 7)
			{
				...
				gridColumns = [
				               	...
				               	{header: "{/literal}{#str_LabelCompany#}{literal}", width: 150, hidden: companyColumnHidden, renderer: companyCodeRenderer, dataIndex: 'companycode'},
				    			{header: "{/literal}{#str_LabelLicenseKey#}{literal}", width: 150, dataIndex: 'lkey', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeStart#}{literal}", width: 90, dataIndex: 'qrs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeEnd#}{literal}", width: 90, dataIndex: 'qre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_ComponentPriceRangeStart#}{literal}", width: 125, dataIndex: 'crs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_ComponentPriceRangeEnd#}{literal}", width: 125, dataIndex: 'cre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PriceRangeBasePrice#}{literal}", width: 70, dataIndex: 'bp', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PriceRangeUnitPrice#}{literal}", width: 70, dataIndex: 'up', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_LabelLineSubtract#}{literal}", width: 90, dataIndex: 'ls', renderer: generalColumnRenderer},
								{id: 'includestax', header: "{/literal}{#str_LabelIncludesTax#}{literal}", width: 160, dataIndex: 'it', renderer: generalColumnRenderer},
			...
			else if (selectedPricingModel == 8)
			{
				...
				gridColumns = [
				               	...
				               	{header: "{/literal}{#str_LabelCompany#}{literal}", width: 150, hidden: companyColumnHidden, renderer: companyCodeRenderer, dataIndex: 'companycode'},
				    			{header: "{/literal}{#str_LabelLicenseKey#}{literal}", width: 150, dataIndex: 'lkey', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeStart#}{literal}", width: 80, dataIndex: 'qrs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_QtyPriceRangeEnd#}{literal}", width: 80, dataIndex: 'qre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_ComponentPriceRangeStart#}{literal}", width: 80, dataIndex: 'crs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_ComponentPriceRangeEnd#}{literal}", width: 80, dataIndex: 'cre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PageCountRangeStart#}{literal}", width: 80, dataIndex: 'prs', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PageCountRangeEnd#}{literal}", width: 80, dataIndex: 'pre', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PriceRangeBasePrice#}{literal}", width: 60, dataIndex: 'bp', renderer: generalColumnRenderer},
								{header: "{/literal}{#str_PriceRangeUnitPrice#}{literal}", width: 60, dataIndex: 'up', renderer: generalColumnRenderer},
			...
			gridProductConfigPricingDataStoreObj = new Ext.data.GroupingStore(
			{
				...
				proxy: new Ext.data.HttpProxy({url: 'index.php?fsaction=AdminProducts.getProductsConfigPricingGridData&ref={/literal}{$ref}{literal}'}),
	...
	function pricingHandler()
	{
		...
		if (selectedNode.attributes.initialized == '0')
		{
			...
			Ext.Ajax.request({
			   url: 'index.php?fsaction=AdminProducts.getProductsConfigPricingGridData&ref={/literal}{$ref}{literal}',
			   success: ajaxSuccess,
			   failure: ajaxFailure,
	...
	function closeConfigWindow()
	{
		...
		{
			Ext.MessageBox.confirm("{/literal}{#str_LabelCurrentlyInEditMode#}{literal}", "{/literal}{#str_MessageCurrentlyInEditMode#}{literal}", processResult);
		}
	...
	{
		Ext.MessageBox.show({ title: "{/literal}{#str_TitleError#}{literal}", msg: "{/literal}{#str_ErrorCouldNotRefreshProductTree#}{literal}", buttons: Ext.MessageBox.OK, icon: Ext.MessageBox.WARNING });
	}

	function getLinkedProductCodeFailure()
	{
		Ext.MessageBox.show({ title: "{/literal}{#str_TitleError#}{literal}", msg: "{/literal}{#str_ErrorCouldNotGetLinkedProduct#}{literal}", buttons: Ext.MessageBox.OK, icon: Ext.MessageBox.WARNING });
	}
	...
	var productLinkingStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: 'index.php?fsaction=AdminProducts.productLinkingList&ref={/literal}{$ref}{literal}', method: 'GET'}),
		...
		baseParams: { companycode: selectedCompanyCode, productcode: '{/literal}{$productcode}{literal}' }
	});
	...
	var productLinkingCombo = new Ext.form.ComboBox({
		...
		listeners:{ 'select': function(theComboBox, selectedRecord, theIndex){
				...
				{/literal}{*Since we are resetting the tree we want to remove any deletions scheduled*}{literal}
				gDeletedItemsArray = [];
				...
				if (selectedProductLinkID == 0)
				{
					Ext.Ajax.request({
							   ...
							   success: productTreeRefreshSuccess,
							   failure: productTreeRefreshFailure,
							   params: { productid: '{/literal}{$productid}{literal}', companycode: companyCombo.getValue()},
							   method: 'GET'
							});
						...
						{/literal}{*make sure the trees are editable*}{literal}
						Ext.getCmp('send_to_top').enable();
						Ext.getCmp('send_to_Bottom').enable();
						Ext.getCmp('move_up').enable();
						Ext.getCmp('move_down').enable();
				...
				else
				{
					Ext.Ajax.request({
						...
						{/literal}{*disable the trees as no changes can be made*}{literal}
						Ext.getCmp('send_to_top').disable();
						Ext.getCmp('send_to_Bottom').disable();
						Ext.getCmp('move_up').disable();
						Ext.getCmp('move_down').disable();
				...
				{/literal}{*close the pricing window as it needs reconfiguring for the new tree*}{literal}
				if (gPricingWindowExists)
				{
					Ext.getCmp('productConfigPricingGridWindow').close();
					gPricingWindowExists = false;
			...
			{
				{/literal}{*prevent the user being able to select an already linked product*}{literal}
		...
		value: '{/literal}{$linkedproductcode}{literal}',
	...
	var categoriesStore = new Ext.data.Store({
		id: 'categoriesStore',
		proxy: new Ext.data.HttpProxy({url: 'index.php?fsaction=AjaxAPI.callback&cmd=COMPONETCATEGORYLIST&ref={/literal}{$ref}{literal}&companycode='+selectedCompanyCode, method: 'GET'}),
		reader: new Ext.data.ArrayReader({
			idIndex: 0},
		Ext.data.Record.create([
		    {name: 'id', mapping: 0},
	...
	var categoriesCombo = new Ext.form.ComboBox({
		...
		value: '{/literal}{#str_LabelSections#}{literal}',
		useID: true,
		post: true
	});
	...
	var tree = new Ext.tree.TreePanel({
		 ...
		 tbar: [
				 {
					...
					text: "{/literal}{#str_LabelRemove#}{literal}",
					iconCls: 'silk-delete',
				...
				},'-',
				{
					...
					text: "{/literal}{#str_LabelDefault#}{literal}",
					iconCls: 'silk-tick',
					disabled: true,
		 ...
		 loader: new Ext.tree.TreeLoader({
             dataUrl:'index.php?fsaction=AdminProducts.refreshProductTree&ref={/literal}{$ref}{literal}',
	...
	var root = new Ext.tree.AsyncTreeNode({
     ...
     var tree2 = new Ext.tree.TreePanel({
         ...
         lines: false,
         loader: new Ext.tree.TreeLoader({
             dataUrl:'index.php?fsaction=AdminProducts.getComponentsFromCategory&ref={/literal}{$ref}{literal}',
             preLoadChildren: true,
			 baseParams: {csrf_token: Ext.taopix.getCSRFToken()}
         }),
         enableDrop:false,
     ...
     Ext.getCmp('tree2').loader.on("beforeload", function(treeLoader, node) {
        Ext.getCmp('tree2').loader.baseParams.companycode = '{/literal}{$companycode}{literal}';
    }, this);
        ...
        items: [
        	...
        	items:[
				...
				{/literal}{/if}{literal}
				{
					...
					align: 'middle',
					items: [
						{xtype: 'label', text: "{/literal}{#str_LabelLinkedProductConfiguration#}{literal}:", forId: 'productlinkingcombo', cls: 'x-form-item', style: 'padding: 2px'},
    ...
    gDialogObj = new Ext.Window({
		id: 'productConfigDialog',
	 	...
	 	resizable:false,
	  	...
	  	buttons:
		[
		 	{id:'editconfig', text: '{/literal}{#str_ButtonEdit#}{literal}', iconCls: 'silk-pencil', handler: editConfigHandler, cls: 'x-btn-left', ctCls: 'width_100'},
			{ text: "{/literal}{#str_ButtonCancel#}{literal}", id: 'cancelconfig', handler: editConfigHandler, disabled: true},
			{ id: 'addEditButton', text: "{/literal}{#str_ButtonSave#}{literal}", disabled: true, handler: addsaveHandler}
		]
	});
