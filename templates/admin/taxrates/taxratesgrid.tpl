...
{	
	...
	{	
		if(!taxRateEditWindowExists)
		{
			taxRateEditWindowExists = true;
			Ext.taopix.loadJavascript(gMainWindowObj, '', 'index.php?fsaction=AdminTaxRates.addDisplay&ref={/literal}{$ref}{literal}', '', '', 'initialize', false);
		}
	...
	function onEdit(btn, ev)
	{
		...
		{
			taxRateEditWindowExists = true;
			Ext.taopix.loadJavascript(gMainWindowObj, '', 'index.php?fsaction=AdminTaxRates.editDisplay&ref={/literal}{$ref}{literal}', serverParams, '', 'initialize', false);
		}
	}
	...
	function onDeleteResult(btn)
	{
		if (btn == "yes")
		{
			...
			Ext.taopix.formPost(gMainWindowObj, paramArray, 'index.php?fsaction=AdminTaxRates.delete', "{/literal}{#str_MessageDeleting#}{literal}", onDeleteCallback);	
		}
	}
	...
	var gridDataStoreObj = new Ext.data.Store({
		remoteSort: true, 
		proxy: new Ext.data.HttpProxy({url: 'index.php?fsaction=AdminTaxRates.getGridData&ref={/literal}{$ref}{literal}'}),
	...
	var gridColumnModelObj = new Ext.grid.ColumnModel({
		...
		columns: [
			...
			{header: "{/literal}{#str_LabelRate#}{literal}", width: 200, dataIndex: 'rate', align: 'right'}
		]
	});
	...
	gMainWindowObj = new Ext.Panel({
		id: 'MainWindow',
		title: "{/literal}{#str_TaxTitleTaxRates#}{literal}",
		items: grid,
		layout: 'fit',
		anchor: '100% 100%',
