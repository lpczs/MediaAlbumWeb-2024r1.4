...
function initialize(pParams)
{
	...
	function onAddPriceList(btn, ev)
	{
		...
		if (!productPriceListEditWindowExists)
		{
			...
			Ext.taopix.loadJavascript(gComponentsPriceListDialogObj, '', 'index.php?fsaction=AdminProductPricing.priceListAddDisplay&ref={/literal}{$ref}{literal}', serverParams, '', 'initialize', false);
		}
	}
	...
	function onEditPriceList(btn, ev)
	{
		...
		{
			productPriceListEditWindowExists = true;
			Ext.taopix.loadJavascript(gComponentsPriceListDialogObj, '', 'index.php?fsaction=AdminProductPricing.priceListEditDisplay&ref={/literal}{$ref}{literal}', serverParams, '', 'initialize', false);
		}
	}
	...
	function onDeletePriceList(btn, ev)
	{
		...
		function onDeletePriceListResult(btn)
		{
			if (btn == "yes")
			{
				...
				Ext.taopix.formPost(Ext.getCmp('componentPriceListDialog'), paramArray, 'index.php?fsaction=AdminProductPricing.priceListDelete&ref={/literal}{$ref}{literal}', "{/literal}{#str_MessageDeleting#}{literal}", onDeletePriceListCallback);
			}
		}
		...
		var message = nlToBr("{/literal}{#str_DeleteProductPriceListConfirmation#}{literal}".replace("^0", codeList));
		dataStore.load();
	...
	function activatePriceList(btn, ev)
	{
		...
		Ext.taopix.formPost(Ext.getCmp('componentPriceListDialog'), serverParams, 'index.php?fsaction=AdminProductPricing.activatePriceList&ref={/literal}{$ref}{literal}', "{/literal}{#str_MessageUpdating#}{literal}", priceListActivateCallback);
	}
	...
	var gridPriceListDataStoreObj = new Ext.data.GroupingStore({
		...
		{/literal}{/if}{literal}
		proxy: new Ext.data.HttpProxy({url: 'index.php?fsaction=AdminProductPricing.getPriceListGridData&category='+category+'&pricingmodel={/literal}{$pricingModel}{literal}&ref={/literal}{$ref}{literal}'}),
	...
	var priceListGridColumnModel = new Ext.grid.ColumnModel({
		...
		columns: [
			...
			{/literal}{if $pricingModel == 3}{literal}
				...
				{header: "{/literal}{#str_LabelStatus#}{literal}", renderer: statusRenderer, width: 45, dataIndex: 'active'}
			{/literal}{elseif $pricingModel == 5}{literal}
				...
				{header: "{/literal}{#str_SidePriceRangeStart#}{literal}", width: 90, dataIndex: 'srs', renderer: generalColumnRenderer},
				{header: "{/literal}{#str_SidePriceRangeEnd#}{literal}", width: 90, dataIndex: 'sre', renderer: generalColumnRenderer},
				{header: "{/literal}{#str_PriceRangeBasePrice#}{literal}", width: 80, dataIndex: 'bp', renderer: generalColumnRenderer},
				{header: "{/literal}{#str_PriceRangeUnitPrice#}{literal}", width: 80, dataIndex: 'up', renderer: generalColumnRenderer},
	...
	var gComponentsPriceListDialogObj = new Ext.Window({
		...
		title: "{/literal}{#str_LabelPriceLists#}{literal}",
		closable:false,
		closeAction: 'close',
		tools:
		[
