...
{literal}
		...
		window.onload = function()
        {
			...
			addEventListener("keydown", function (e)
			{
				if (e.keyCode == 13)
				{
					...
					{
						case "":
						case "#resetpasswordauthcodesubmit":
							validateAuthCodeDataEntry();
							break;
						case "#forgotpassword":
			...
			{
				// load the main page
				if (window.location.hash == '#resetpasswordauthcodesubmit')
				{
					gPreviousHash = window.location.hash;
				}
				else if(window.location.hash == '#resetpasswordsubmit')
				{
					// Reset to the initial load.
				   ...
				   switch (obj)
				   {
						...
						case 'resetpasswordauthcodesubmit':
							...
							else
							{
								...
								if (gPreviousHash == '#resetpasswordauthcodesubmit')
								{
									initializeAuthForm();
								...
								{
									setHashUrl('resetpasswordsubmit');
								}
							}
							break;
						case 'resetpasswordsubmit':
		...
		*/
		...
		function validateDataEntryForgotPassword()
		{
         ...
         // wrapper function for validateAuthCodeDataEntry
        function fnValidateAuthCodeDataEntry(pElement)
        {
            return validateAuthCodeDataEntry();
        }

		function validateAuthCodeDataEntry()
		{
			gAlerts = 0;
			var message = "{/literal}{#str_ErrorCompulsoryInformationMissing#}{literal}";
			...
			else
			{
				...
				// send the ajax
				var postParams = '&data1=' + authcode.value;
				postParams += '&data2=' + '{/literal}{$requesttoken}{literal}';
				postParams += '&mobile=true';
				...
				processAjaxSmallScreen("resetpasswordauthcodesubmit",".?fsaction=Welcome.resetPasswordProcessAuthCode", 'POST', postParams);
			}
		}
		...
		function initializeForgotPassword()
		{
			...
			{
				toggleNewPasswordElement.removeEventListener('click', toggleNewPassword);
				toggleNewPasswordElement.addEventListener('click', toggleNewPassword);
			}
		}

		function toggleNewPassword()
		{
			togglePasswordVisibility(document.getElementById('togglenewpassword'), 'newpassword');
		}
                        ...
                        <div class="contentVisible">
                            ...
                            <div id="contentHolder">
								...
								<div>
									...
									<div class="formLine1">
										<label for="login2">{#str_LabelEnterAuthenticationCode#}:</label>
										<img src="{$brandroot}/images/asterisk.png" alt="*"/>
									</div>

									<div class="formLine2">
										<input type="text" id="authcode" name="authcode" value="" class="middle" autocapitalize="off"/>
										<img class="error_form_image" id="authcodecompulsory" src="{$brandroot}/images/asterisk.png" alt="*" />
										<div class="clear"></div>
									</div>
							...
							<div class="paddingBtnBottomPage">
								<div class="btnAction btnContinue" data-decorator="fnValidateAuthCodeDataEntry">
									<div class="btnContinueContent">{#str_ButtonNext#}</div>
								</div>
							</div>

							<p>
                            	{#str_LabelLostYourAuthCode#}
                            	<a href="#" id="forgotPasswordLink" style="font-size:14px; color:#3C73C6" data-decorator="fnShowForgotPassword">{#str_LabelStartAgain#}</a>
                            </p>
