...
class ePDQObj
{
    static function configure()
    {
        ...
        $ePDQConfig = PaymentIntegrationObj::readCCIConfigFile('../config/epdq.conf',$gSession['order']['currencycode'],$gSession['webbrandcode']);
        $currencyList = $ePDQConfig['EPDQCURRENCIES'];

        if (($ePDQConfig['SERVER'] == '') || ($ePDQConfig['CLIENTID'] == '') || ($ePDQConfig['PASSPHRASEIN'] == ''))
        {
            $active = false;
        }
    ...
    static function initialize()
    {
        ...
        if ($gSession['order']['ccidata'] == '')
        {
			...
			$server = $ePDQConfig['SERVER'];
			$merchant = $ePDQConfig['CLIENTID'];
			$passPhrase = $ePDQConfig['PASSPHRASEIN'];
			$TITLE = $ePDQConfig['TITLE'];
			$BGCOLOR = $ePDQConfig['BGCOLOR'];
			$TXTCOLOR = $ePDQConfig['TXTCOLOR'];
			$TBLBGCOLOR = $ePDQConfig['TBLBGCOLOR'];
			$TBLTXTCOLOR = $ePDQConfig['TBLTXTCOLOR'];
			$BUTTONBGCOLOR = $ePDQConfig['BUTTONBGCOLOR'];
			$BUTTONTXTCOLOR = $ePDQConfig['BUTTONTXTCOLOR'];
			$LOGO = $ePDQConfig['LOGO'];

			//Initialise variables
			$defaultLanguage = $gConstants['defaultlanguagecode'];
			$orderID = $gSession['ref'] . '_'. time();

			if (array_key_exists('ORDERDESCRIPTION', $ePDQConfig))
        	{
				if ($ePDQConfig['ORDERDESCRIPTION'] == '')
				{
					$orderData = '';
				}
				else
				{
					$orderData = UtilsObj::LeftChars($ePDQConfig['ORDERDESCRIPTION'], 20);
				}
			}
			else
			{
			...
			// Passing empty values in the array causes ePDQ to fail.
			...
			// set the ccidata to remember we have jumped to ePDQ
    ...
    static function confirm($callback)
    {
        ...
        $update = false;
     	...
     	$ePDQConfig = PaymentIntegrationObj::readCCIConfigFile('../config/ePDQ.conf',$gSession['order']['currencycode'],$gSession['webbrandcode']);
		$passPhraseOut = $ePDQConfig['PASSPHRASEOUT'];
		$logFilePath = $ePDQConfig['LOGFILEPATH'];
        $logOutput = $ePDQConfig['LOGOUTPUT'];
		...
		PaymentIntegrationObj::logPaymentGatewayData($ePDQConfig, $serverTimestamp);
