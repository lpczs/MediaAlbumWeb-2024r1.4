...
do {
	...
	echo "Processing shared items batch " . $counter . " ...\n";

	$getSharedItemsDataResult = getSharedItems($startRecordID);

	if ($getSharedItemsDataResult['error'] == '')
	{
		$updateSharedItemsPasswordResult = updateSharedItemPassword($getSharedItemsDataResult['data']['items']);

		if ($updateSharedItemsPasswordResult['result'] != '')
		{
			$error = $updateSharedItemsPasswordResult['result'];
			$errorParam = $updateSharedItemsPasswordResult['resultparam'];

			echo "\n" . $updateSharedItemsPasswordResult['errorparam'];
		}
	}
	else
	{
		echo "\n" . $getSharedItemsDataResult['errorparam'];
	}

	echo "...Complete\n\n";

	$counter++;
	$startRecordID = $getSharedItemsDataResult['data']['lastrecordid'];
}
...
/**
 * getSharedItems
 *  - get record id and password for updating
 *	  ignore already converted passwords
 */
function getSharedItems($pStartID)
{
	$resultArray = array('error' => '', 'errorparam' =>'', 'data' => array());
	$error = '';
	$errorParam = '';
	$itemListArray = array('items' => array(), 'lastrecordid' => 0);
	...
	if ($dbObj)
	{
		$sql = "SELECT `id`, `password`
				...
				AND SUBSTR(`password`, 1, 1)  != '+'
				LIMIT " . TPX_PASSWORDRECORD_BATCH_SIZE;

		if ($stmt = $dbObj->prepare($sql))
		{
			if ($stmt->bind_param('i',  $pStartID))
			{
				if ($stmt->bind_result($recordID, $password))
				{
					if ($stmt->execute())
					{
						while($stmt->fetch())
						{
							...
							$itemListArray['items'][$recordID] = $item;
							$itemListArray['lastrecordid'] = $recordID;
						}
					}
					else
					{
	...
	$resultArray['data'] = $itemListArray;

	return $resultArray;
}

/**
 * updateSharedItemPassword
 *  - update shared item passwords to new format
 *	  ignore any passwords that might have already been converted
 */
function updateSharedItemPassword($pSharedItemsDataArray)
{
	$result = '';
	$resultParam = '';
	...
	if ($dbObj)
	{
		...
		$sql = "UPDATE `SHAREDITEMS` SET `password` = ?
				WHERE `id` = ?
					AND SUBSTR(`password`, 1, 1) != '$'
					AND SUBSTR(`password`, 1, 1)  != '+'";
		...
		if ($stmt = $dbObj->prepare($sql))
		{
			foreach($pSharedItemsDataArray as $sharedItem)
			{
				$result = '';
				$resultParam = '';

				$generatePasswordHashResult = AuthenticateObj::generatePasswordHash($sharedItem['password'], TPX_PASSWORDFORMAT_MD5);

				if ($generatePasswordHashResult['result'] == '')
				{
					if ($generatePasswordHashResult['data'] != '')
					{
						$sharedItem['password'] = $generatePasswordHashResult['data'];
					}
					else
					{
						$result = 'str_Error';
				...
				if ($result == '')
				{
					if ($stmt->bind_param('si', $sharedItem['password'], $sharedItem['id']))
					{
						if (! $stmt->execute())
						{
