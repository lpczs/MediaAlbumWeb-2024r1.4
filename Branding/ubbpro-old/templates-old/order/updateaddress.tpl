    ...
    var gAs_jsonState = '';
    var session = "{$session}";
    var firstname = "{$contactfname}";
    var lastname = "{$contactlname}";
    ...
    var registeredtaxnumber = "{$registeredtaxnumber}";
...
{literal}
    function initializeAddress(pIsSmallScreen, pBackButton)
    {
        ...
        processAjaxAddress("ajaxdiv",".?fsaction=AjaxAPI.callback&cmd=ADDRESSFORM&ref=" + session + "&country=" + country + "&hideconfigfields=" + hideConfigFields + "&strict=1&edit={/literal}{$edit}{literal}", {/literal}{$useraddressupdated}{literal}, 'GET', '');
    ...
    function cancelDataEntry()
    {
        ...
        {if $issmallscreen == 'true'}
            ...
            processAjaxAddress("cancel",".?fsaction=AjaxAPI.callback&cmd=CHANGEADDRESSCANCEL&ref=" + session, 1, 'POST', postParams);
...
{literal}
        ...
        if ((document.getElementById("emailcompulsory")) && (document.getElementById("emailcompulsory").src.indexOf("/images/asterisk.png") != -1))
        {
            if (email.length == 0)
            {
        ...
        if ((document.getElementById("email2compulsory")) && (document.getElementById("email2compulsory").src.indexOf("/images/asterisk.png") != -1))
        {
            if (email.length == 0)
            {
                message += "\n" + "{/literal}{#str_MessageCompulsoryEmaiInvalid#}{literal}";
...
{literal}
        ...
        {if $issmallscreen == 'true'}
            ...
            if (gCurrentSource == 'jobticket')
            {
                ...
                var postParams = '&contactfname=' + firstname;
                ...
                postParams += '&countryname=' + document.submitformaddress.countryname.value ;
                ...
                postParams += '&telephonenumber=' + telephonenumber;
                postParams += '&email=' + document.submitformaddress.email.value;
				...
				{if ($useraddressupdated == 0 || $useraddressupdated == 2)}
					...
					{literal}
						processAjaxAddress("refreshshippingPanel",".?fsaction=AjaxAPI.callback&cmd=UPDATEACCOUNTDETAILS&ref=" + session, 0, 'POST', postParams);
					{/literal}
				...
				{else}
					...
					if (gAddressType == "shipping")
					{
						processAjaxAddress("refreshshippingPanel",".?fsaction=AjaxAPI.callback&cmd=CHANGESHIPPINGADDRESS&ref=" + session, 0, 'POST', postParams);
					}
					else
					{
						processAjaxAddress("refreshshippingPanel",".?fsaction=AjaxAPI.callback&cmd=CHANGEBILLINGADDRESS&ref=" + session, 0, 'POST', postParams);
					}
					{/literal}
				{/if}
				{literal}
    ...
    function setCountry()
    {
        ...
        saveFields();
        countryChanged = true;
        processAjaxAddress("ajaxdiv",".?fsaction=AjaxAPI.callback&cmd=ADDRESSFORM&ref=" + session + "&country=" + country + "&hideconfigfields=" + hideConfigFields +"&strict=1&edit={/literal}{$edit}{literal}", 1, 'GET', '');
    }
    ...
    function verifyAddress()
    {
        ...
        processAjaxAddress("verify",".?fsaction=AjaxAPI.callback&cmd=ADDRESSVERIFICATION&ref=" + session + "&city=" + city +
        "&county=" + county + "&statecode=" + regioncode + "&statevalue=" + state + "&postcode=" + postcode + "&region=" + region +
        "&country=" + country, 1, 'GET', '');
            ...
            xmlhttp.onreadystatechange = function()
            {
                ...
                if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
                {
                    ...
                    {if $issmallscreen == 'true'}
                        ...
                        createDialog("{/literal}{#str_TitleWarning#}{literal}", "{/literal}{#str_MessageUpdateAddressDetails#}{literal}", "closeDialog()");
                ...
                if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
                {
                    switch (obj)
                    {
                        case 'refreshshippingPanel':
                            ...
                            // change javascript
                            toggleJs('mainjavascript', jsonObj.javascript, true, '');
                            window.history.back();
                        break;
                        case 'cancel':
                            var jsonObj = eval('(' + xmlhttp.responseText + ')');
