 ...
 * @package Security
 *
 * Taken from https://github.com/paragonie/csp-builder/ and modified slightly
 */
class CSPBuilder
{
    ...
    public function saveSnippet(
        string $outputFile,
        string $format = self::FORMAT_APACHE
    ): bool {
        if ($this->needsCompile) {
            $this->compile();
        }
     ...
     * @return array
     */
    public function getPoliciesArray(): array
    {
     ...
     * @return string
     */
    public function getPoliciesJson() : string
    {
        return \json_encode($this->policies);
    }
}