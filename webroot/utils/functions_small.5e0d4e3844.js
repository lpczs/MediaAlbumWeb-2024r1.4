...
*/
function toggleJs(pID, pScript, pInclude, pSrc, pNonce, pScriptAfterLoad, pHash, pForce)
{
    if ('undefined' === typeof pForce)
	{
		pForce = false;
	}
    ...
    if (pInclude)
    {
        ...
        {
            /**
             * if there is already an existing script element &
             * the src matches the argument src, then we want
             * to return to prevent re-running the same javascript
             * again
             **/
            if ('' !== contentCustomScript.src && contentCustomScript.src === pSrc && !pForce) {
                /**
                 * check to see if there is also an inline script to be added
                 * to be executed onload of the src, and compare the hashes
                 * to see if it is already loaded
                 */
                if ('' !== pScriptAfterLoad && ('undefined' !== typeof pHash && '' !== pHash)) {
                    var additional = document.getElementById('additionaljavascript_' + pID);
                    if (additional && '' !== additional.integrity) {
                      if (additional.integrity === pHash) {
                        return
                      }
                    }
                }
            ...
            oHead.removeChild(contentCustomScript);
        }

        // Make sure a script needs to be loaded.
        if (((pScript !== undefined) && (pScript !== '')) || ((pSrc !== undefined) && (pSrc !== '')))
        {
            ...
            if ('' !== pNonce)
            {
                oScript.nonce = pNonce;
            }

            if (pScript !== '')
            {
                // If we have a nonce tag to replace we need to do so here.
                if ((pScript !== undefined) && (pScript.indexOf("[nonce]") !== -1))
                {
                    pScript = pScript.replace("[nonce]", pNonce);
                }

                oScript.text = pScript;

                /**
                 * only add the script integrity if a has has been
                 * passed in an is inline, so we can compare hashes
                 * so we don't re-execute code (mostly lightbox gateways)
                 */
                if ('undefined' !== typeof pHash && '' !== pHash) {
                    oScript.integrity = pHash;
                }
            }
            else
            {
                ...
                oScript.onload = oScript.onreadystatechange = function()
                {
                    if ( !r && (!this.readyState || this.readyState === 'complete') )
                    {
                        ...
                        if (pScriptAfterLoad !== '')
                        {
                            toggleJs('additionaljavascript_' + pID, pScriptAfterLoad, true, '', pNonce,'', pHash);
                        }
                    }
                };
            }
...
{
	gKeepDialogOpen = true;
	
	// make sure the animation is not alreday on screen
	if (document.getElementById('dialogLoading').style.display != 'block')
	{
...
{
	gKeepDialogOpen = false;

    // delay the close function
    setTimeout(function()
    {
		// make sure another action has not tried to reopen the dialog
		if (! gKeepDialogOpen)
		{
			...
			if (dialogLoading)
			{
                ...
                if (typeof gLoadingImage.stop === 'function')
                {
                    gLoadingImage.stop();
                }
			}
...
function createDialog(pTitle, pContent, pClickAction, pUniqueClass, pContentClass)
{
    ...
    openDialog(dialogContent, pClickAction);
}
...
function generateDialogContent(pTitle, pContent, pHasCancel)
{
    ...
    if (pHasCancel)
    {
        dialogContent +='<div class="btnLeftSection btnInside" id="dialogCancelBtn" data-decorator="closeDialog">';
...
function showConfirmDialog(pTitle, pContent, pCallBack)
{
    ...
    openDialog(dialogContent, pCallBack);
}
...
*/
function openDialog(pContent, pCallback)
{
    gDialogStatus = 'open';
    ...
    if (dialogOuter)
    {
        ...
        setDialogPosition(pCallback);
    }
}

function setDialogPosition(pCallback)
{
    var dialogOuter = document.getElementById('dialogOuter');
    ...
    if (dialogBtn) {
      dialogBtn.addEventListener('click', pCallback);
    }
...
function setContentVisibleHeight(pContainer)
{
    ...
    {
        // make sure we still are in same panel as the action which open the keyboard
...
{
    var jsonObj = eval('(' + pResponseText + ')');
