                ...
                if ($stmt->execute()) {
                    while ($stmt->fetch()) {
                        if ($this->callInternal($id)) {
                            echo "brand ".('' !== $code ? $code : 'default brand')." has been copied to the online server" . PHP_EOL;
                        } else {
                            echo "There was an error copying brand $code to the online server" . PHP_EOL;
                        }
                    }
                }
            }
